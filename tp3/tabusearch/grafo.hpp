...
class Grafo {
    public:
        ...
        set<uint> maxClique(uint maxIteraciones);
    ...
    private:
        ...
        uint elegirCandidatoContraccion(const set<uint>& s, const vector<uint>& vecesUsado);
        //devuelve un candidato para expandir.
        uint elegirCandidatoExpansion(const list<uint>& s, const vector<uint>& vecesUsado);
