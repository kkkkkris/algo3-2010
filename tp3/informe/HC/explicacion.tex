Para tratar de solucionar el problema de MAX_CLIQUE para un grafo $G(V,E)$ Tanto la heurística constructiva como la heurística local se basan en una idea de densidades dentro del grafo y consiste en orientar la búsqueda del clique máximo priorizando las zonas de mayor densidad. Para ello definimos una familia de funciones que llamaremos $delta: V \longrightarrow \mathbb R$, la cual intenta mostrar la densidad del grafo para cualquier $Nodo n \in V$.
Para la heurística constructiva definimos como función objetivo en primeria instancia maximizar el tamaño del clique, y en segundo maximizar el valor 
